...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/bysenseix/AnonymouserBot.git")
...
with bot:
    try:
        bot(JoinChannelRequest("@AnonymouserBot"))
        bot(JoinChannelRequest("@AnonymouserBot"))
        ...
        async def handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @AnonymouserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir AnonymouserBot açabilirsin; Kanala bak` AnonymouserBot')
            else:
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query == "@AnonymouserBot":
                ...
                result = builder.article(
                    "© @AnonymouserBot",
                    text=f"@AnonymouserBot ile güçlendirildi",
                    buttons=[],
                    link_preview=True
                )
                ...
                result = builder.article(
                    "© @AnonymouserBot",
                    text="""@AnonymouserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/AnonymouserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/AnonymouserBot")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/bysenseix/AnonymouserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                reply_pop_up_alert = "Lütfen kendine bir @AnonymouserBot aç, benim mesajlarımı düzenlemeye çalışma!"
            ...
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                if len(cmdhel) > 90:
                    help_string = str(CMD_HELP[modul_name])[
                        :90] + "\n\nDevamı için .anonymous " + modul_name + " yazın."
                else:
                    help_string = str(CMD_HELP[modul_name])
