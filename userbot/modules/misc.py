...
async def randomise(items):
    ...
    if len(itemo) < 2:
        await items.edit(
            "`2 veya daha fazla eşya gerekli. Daha fazla bilgi için .anonymous random komutunu gir.`"
        )
        return
...
async def bot_support(wannahelp):
    ...
    await wannahelp.edit("[Buradan](http://t.me/Noxusizm)")


@register(outgoing=True, pattern="^.creator$")
async def creator(e):
    await e.edit("Bu bot \n"
                 "[NOXUS](https://t.me/Noxusizm) tarafından geliştirilmiştir. \n")


@register(outgoing=True, pattern="^.readme$")
async def reedme(e):
    await e.edit("[AnonymouserBot README.md](https://github.com/bysenseix/AnonymouserBot/blob/master/README.md)")
...
async def repo_is_here(wannasee):
    ...
    await wannasee.edit("[AnonymouserBot Repo](https://github.com/bysenseix/anonymouserbot)")
...
CMD_HELP.update({
    'repo':
    '.repo\
\nKullanım: AnonymouserBot UserBot GitHub reposu'
})
