    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"Sahibim Yüce Noxus Şu an AFK.\
                    \nSebep: `{AFKREASON}`")
                else:
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"Yüce Noxus Artık AFK.\
        \nSebep: `{string}`")
    else:
        await afk_e.edit("Yüce Noxus Artık AFK.")
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        await notafk.respond("Sahibim Yüce Noxus Artık AFK Değil")
        await sleep(2)
        if BOTLOG:
