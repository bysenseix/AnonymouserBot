...
async def ban(bon):
    ...
    if user.id in BRAIN_CHECKER:
        await bon.edit(
            "`Ban Hatası! AnonymouserBot Yetkilisini yasaklayamam.`"
        )
        return
...
async def gspider(gspdr):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await gspdr.edit("`Gmute Hatası! Asena Yetkilisini küresel olarak susturamam.`")
        return
...
async def kick(usr):
    ...
    if user.id in BRAIN_CHECKER:
        await usr.edit(
            "`Kick Hatası! AnonymouserBot Yetkilisini gruptan tekmeleyemem.`"
        )
        return
...
async def warn(event):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await event.edit("`Warn Hatası! AnonymouserBot Yetkilisini uyaramam.`")
        return
